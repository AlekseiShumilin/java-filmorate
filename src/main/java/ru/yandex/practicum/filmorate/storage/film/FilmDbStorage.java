...
public class FilmDbStorage implements FilmStorage {
    ...
    public Optional<Film> updateFilm(Film film) {
        ...
        if (rowsNum == 0) {
            ...
            if (film.getGenres() != null) {
                ...
                Stream<Integer> idStream = film.getGenres().stream()
                        .map(Genre::getId);
                List<Integer> updatedGenresId = idStream.collect(Collectors.toList());
                idStream.close();
    ...
    public Collection<Film> getAllFilms() {
        ...
        Stream<Film> filmStream = jdbcTemplate.queryForStream(sqlQueryfilmIdList, this::mapRowToFilmId)
                .map(this::getFilm)
                .filter(Optional::isPresent)
                .map(Optional::get);
        List<Film> filmList = filmStream.collect(Collectors.toList());
        filmStream.close();
        return filmList;
    }
